...
router.get("/", async (req, res) => {
  try {
    ...
    const findDelivery = DELIVERY_DATES.filter(
      (delivery) => delivery.postal == postalCode
    );
    let foundItems;
    const updateItems =
      //match ids of delivery to items and update delivery according to user input
      items.map((item) => ({
        ...item,
        estimatedDeliveryDate: findDelivery.filter((sect) =>
          item.estimatedDeliveryDate
            .map((id) => parseInt(id))
            .includes(parseInt(sect.id))
        ),
      }));
